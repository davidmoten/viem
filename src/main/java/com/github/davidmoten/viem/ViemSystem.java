...
public interface ViemSystem<K, V, M> {
	 ...
	 * 
	 * <p>
	 * This method should be commutative in that
	 * {@code mergeable(a, b) = mergeable(b, a)}.
	 * 
	...
	/**
	 * Returns the result of merging two metadata objects. For a timed position
	 * report the result might be to use the latest time metadata and copy over any
	 * still relevant information (for example vessel name or destination) in the
	 * older report that has not been obsoleted by the newer report.
	 * 
	 * <p>
	 * This method should be commutative in that {@code merge(a, b) = merge(b, a)}.
	 * 
	 * @param a first
	 * @param b second
	...
	ViemSystem<K, V, M> update(List<EntityState<K, V, M>> matches, Set<EntityState<K, V, M>> newEntityStates);
	...
	default void checkingMetadataMergeability(EntityState<K, V, M> a, EntityState<K, V, M> b) {
